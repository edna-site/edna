...
#	full_path="$(cd "${0%/*}" 2>/dev/null; echo "$PWD"/"${0##*/}")"
	...
	export EDNA_HOME=`dirname "$full_path" | sed 's/\/fullFieldXASv1\/datamodel$//'`
	echo "Guessing EDNA_HOME= "$EDNA_HOME
fi

xsDataBaseName=XSDataFullFieldXAS.xsd
xsdHomeDir=${EDNA_HOME}/fullFieldXASv1/datamodel
pyHomeDir=${EDNA_HOME}/fullFieldXASv1/src
