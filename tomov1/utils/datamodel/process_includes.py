#!/usr/bin/env python
#
# This file belongs to the generateDS project.
#
...
#        doc.write(sys.stdout)
    ...
    if outpath:
        outfile = make_file(outpath)
        if outfile:
            doc.write("tmp_" + outfile.name )
            fTemp = open( "tmp_" + outfile.name )
            strContent = fTemp.read()
            strContent = str.replace( strContent, "ns0", "xs", -1 )
