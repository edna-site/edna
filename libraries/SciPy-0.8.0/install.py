...
scipyLibrary = "scipy-0.8.0.tar.gz"
numpyDir = "NumPy-1.4.1"
blasLibrary = "blas.tgz"
lapackLibrary = "lapack.tgz"
libblas = "libblas.so"
...
#                makeFile.write("RANLIB = ls %s" % (os.linesep))
    ...
    for i in  sys.modules:
         EDVerbose.DEBUG("%s:\t%s" % (i, sys.modules[i]))

    if "PYTHONPATH" in os.environ:
        EDVerbose.DEBUG("PYTHONPATH=%s" % os.environ["PYTHONPATH"])
    else:
        EDVerbose.DEBUG("PYTHONPATH is not Defined !!!!")
...
# ../configure --shared --prefix=~/edna/libraries/20090711-SciPy-0.7.1 --with-netlib-lapack-tarfile=../../lapack.tgz  -Si cputhrchk 0
