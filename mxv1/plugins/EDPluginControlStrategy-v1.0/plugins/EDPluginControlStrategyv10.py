        ...
        if(self.getDataInput().getExperimentalCondition().getBeam().getFlux() is None):
            ...
            if (self.__edPluginRaddose is not None):
                ...
                if(strNumOperators is not None):
                    ...
                    errorMessage = EDMessage.ERROR_EXECUTION_03 % ('EDPluginControlStrategyv10.preProcess', "No symmetry operators found for Space Group: ", strSpaceGroup)
                    EDVerbose.error(errorMessage)
                    self.addErrorMessage(errorMessage)
                    raise RuntimeError, errorMessage
                ...
                try:
                    ...
                    warningMessage = EDMessage.WARNING_CANNOT_USE_PLUGIN_03 % ('EDPluginControlStrategyv10.preProcess', self.__strPluginRaddoseName, self.__edHandlerXSDataRaddose.getClassName() + ": " + str(detail))
                    EDVerbose.warning(warningMessage)
                    self.addWarningMessage(warningMessage)
...
#                        
#                    except Exception, detail:
#                        warningMessage = EDMessage.WARNING_CANNOT_USE_PLUGIN_03 % ('EDPluginControlStrategyv10.preProcess', self.__strPluginRaddoseName, str( detail ) )
#                        EDVerbose.warning( warningMessage )
#                        self.addWarningMessage( warningMessage )

            else:
                errorMessage = EDMessage.ERROR_PLUGIN_NOT_LOADED_02 % ('EDPluginControlStrategyv10.preProcess', self.__strPluginRaddoseName)
                EDVerbose.error(errorMessage)
                self.addErrorMessage(errorMessage)
                raise RuntimeError, errorMessage
        ...
        if (self.__edPluginBest is None):
            errorMessage = EDMessage.ERROR_PLUGIN_NOT_LOADED_02 % ('EDPluginControlStrategyv10.preProcess', self.__strPluginBestName)
            EDVerbose.error(errorMessage)
            self.addErrorMessage(errorMessage)
            raise RuntimeError, errorMessage
        else:
    ...
    def configure(self):
        ...
        if(pluginConfiguration == None):
            warningMessage = EDMessage.WARNING_NO_PLUGIN_CONFIGURATION_ITEM_FOUND_02 % ('EDPluginControlStrategyv10.configure', self.getPluginName())
            ...
            if(strSymopHome == None):
                warningMessage = EDMessage.WARNING_NO_PARAM_CONFIGURATION_ITEM_FOUND_03 % ('EDPluginControlStrategyv10.configure', self.__strCONF_SYMOP_HOME, self.getPluginName())
                EDVerbose.warning(warningMessage)
                self.addWarningMessage(warningMessage)
            else:
    ...
    def process(self, _edObject=None):
        ...
        EDVerbose.DEBUG("EDPluginControlStrategyv10.process...")
    ...
    def postProcess(self, _edObject=None):
        ...
        EDVerbose.DEBUG("EDPluginControlStrategyv10.postProcess...")

        xsDataBestOutput = self.__edPluginBest.getDataOutput()
        xsDataStrategyResult = None

        if(xsDataBestOutput is not None):
            xsDataStrategyResult = self.__edHandlerXSDataBest.getXSDataStrategyResult(xsDataBestOutput, self.getDataInput().getExperimentalCondition(), self.__xsDataSampleCopy)

        self.setDataOutput(xsDataStrategyResult)


    def doRaddoseToBestTransition(self, _edPlugin):
        EDVerbose.DEBUG("EDPluginControlStrategyv10.doRaddoseToBestTransition")
        self.retrieveSuccessMessages(_edPlugin, "EDPluginControlStrategyv10.doRaddoseToBestTransition")
        ...
        from EDHandlerXSDataBestv10    import EDHandlerXSDataBestv10
        self.__edHandlerXSDataBest = EDHandlerXSDataBestv10()

        xsDataBestInput = self.__edHandlerXSDataBest.getXSDataBestInput(self.getDataInput().getExperimentalCondition().getBeam(),
        ...
        self.__edPluginBest.setDataInput(xsDataBestInput)
    ...
    def doFailureActionRaddose(self, _edPlugin):
        ...
        """
        EDVerbose.DEBUG("EDPluginControlStrategyv10.doFailureActionRaddose")
        self.retrieveFailureMessages(_edPlugin, "EDPluginControlStrategyv10.doFailureActionRaddose")
        warningMessage = EDMessage.WARNING_CANNOT_USE_PLUGIN_03 % ('EDPluginControlStrategyv10.doFailureActionRaddose', self.__strPluginRaddoseName, "Raddose failure")
        EDVerbose.warning(warningMessage)
        self.addWarningMessage(warningMessage)
    ...
    def doSuccessActionBest(self, _edPlugin):
        ...
        retrieve the potential warning messages
        """
        EDVerbose.DEBUG("EDPluginControlStrategyv10.doSuccessActionBest")
        self.retrieveSuccessMessages(_edPlugin, "EDPluginControlStrategyv10.doSuccessActionBest")
    ...
    def doFailureActionBest(self, _edPlugin):
        ...
        retrieve the potential error messages
        """
        EDVerbose.DEBUG("EDPluginControlStrategyv10.doFailureActionBest")
        self.retrieveFailureMessages(_edPlugin, "EDPluginControlStrategyv10.doFailureActionBest")
    ...
    def generateExecutiveSummary(self, _edPlugin):
        ...
        """
        EDVerbose.DEBUG("EDPluginControlStrategyv10.generateExecutiveSummary")
        self.addExecutiveSummaryLine("Summary of Strategy:")
        self.addErrorWarningMessagesToExecutiveSummary()

        xsDataStrategyResult = self.getDataOutput()
        ...
        if (self.__edPluginRaddose is not None):
            if (self.__edPluginRaddose.getDataOutput() is not None):
                ...
                xsDataStructure = xsDataStrategyResult.getCollectionPlan()[0].getCollectionStrategy().getSample().getChemicalComposition().getStructure()
