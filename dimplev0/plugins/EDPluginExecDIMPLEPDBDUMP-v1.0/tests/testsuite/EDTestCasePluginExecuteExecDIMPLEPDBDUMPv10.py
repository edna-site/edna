...
class EDTestCasePluginExecuteExecDIMPLEPDBDUMPv10(EDTestCasePluginExecute):
    """Those are all execution tests for the EDNA Exec plugin
    DIMPLEPDBDUMPv10."""
    
    def __init__(self, _edStringTestName = None):
        EDTestCasePluginExecute.__init__(self, "EDPluginExecDIMPLEPDBDUMPv10")

        self.setConfigurationFile( self.getRefConfigFile() )
        
        self.setDataInputFile(EDUtilsPath.mergePath(
            self.getPluginTestsDataHome(),
            "XSDataInputDIMPLEPDBDUMP_reference.xml"))
        
        self.setReferenceDataOutputFile(EDUtilsPath.mergePath(
            self.getPluginTestsDataHome(),
            "XSDataResultDIMPLEPDBDUMP_reference.xml"))

        self.m_edObtainedOutputDataFile = "%s_output.xml" % self.getPluginName()
...
if __name__ == '__main__':
    ...
    test_instance = EDTestCasePluginExecuteControlDIMPLEPDBDUMPv10(
        "EDTestCasePluginExecuteExecDIMPLEPDBDUMPv10")
