...
def fileName2xml(filename):
    ...
    if not os.path.isdir(os.path.join(upperDir, "zimg")):
           os.makedirs(os.path.join(upperDir, "zimg"), int("775", 8))
    destinationPath = os.path.join(upperDir, "zimg", os.path.splitext(os.path.basename(filename))[0] + ".jpg")

    xml = "<XSDataInputExecCommandLine> \
            <commandLineProgram><path ><value>zimg</value></path></commandLineProgram> \
            <commandLineOptions> <value>-a -j</value></commandLineOptions> \
