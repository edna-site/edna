...
#
# Generated Wed May 13 15:28:09 2009 by EDGenerateDS.py.
#
...
class XSDataInputPluginExecSRBRegister(XSDataInput):
    subclass = None
    def __init__(self, srbDropFileName=None, srbURIPattern=None, srbURIReplacement=None, xmlIngestFileName=None, configuration=None):
        XSDataInput.__init__(self, configuration)
        self.srbDropFileName = srbDropFileName
        self.srbURIPattern = srbURIPattern
        self.srbURIReplacement = srbURIReplacement
    ...
    def getSrbURIPattern(self): return self.srbURIPattern
    def setSrbURIPattern(self, srbURIPattern): self.srbURIPattern = srbURIPattern
    def getSrbURIReplacement(self): return self.srbURIReplacement
    def setSrbURIReplacement(self, srbURIReplacement): self.srbURIReplacement = srbURIReplacement
    ...
    def exportChildren(self, outfile, level, name_='XSDataInputPluginExecSRBRegister'):
        ...
        if self.srbURIPattern:
            self.srbURIPattern.export(outfile, level, name_='srbURIPattern')
        if self.srbURIReplacement:
            self.srbURIReplacement.export(outfile, level, name_='srbURIReplacement')
    ...
    def exportLiteralChildren(self, outfile, level, name_):
        ...
        if self.srbURIPattern:
            showIndent(outfile, level)
            outfile.write('srbURIPattern=XSDataString(\n')
            self.srbURIPattern.exportLiteral(outfile, level, name_='srbURIPattern')
            showIndent(outfile, level)
            outfile.write('),\n')
        if self.srbURIReplacement:
            showIndent(outfile, level)
            outfile.write('srbURIReplacement=XSDataString(\n')
            self.srbURIReplacement.exportLiteral(outfile, level, name_='srbURIReplacement')
            showIndent(outfile, level)
            outfile.write('),\n')
        if self.xmlIngestFileName:
            showIndent(outfile, level)
    ...
    def buildChildren(self, child_, nodeName_):
        if child_.nodeType == Node.ELEMENT_NODE and \
            ...
            nodeName_ == 'srbURIPattern':
            obj_ = XSDataString.factory()
            obj_.build(child_)
            self.setSrbURIPattern(obj_)
        elif child_.nodeType == Node.ELEMENT_NODE and \
            nodeName_ == 'srbURIReplacement':
            obj_ = XSDataString.factory()
            obj_.build(child_)
            self.setSrbURIReplacement(obj_)
        elif child_.nodeType == Node.ELEMENT_NODE and \
...
class SaxXSDataDoubleHandler(handler.ContentHandler):
    ...
    def startElement(self, name, attrs):
        ...
        elif name == 'srbURIPattern':
            obj = XSDataString.factory()
            stackObj = SaxStackElement('srbURIPattern', obj)
            self.stack.append(stackObj)
            done = 1
        elif name == 'srbURIReplacement':
            obj = XSDataString.factory()
            stackObj = SaxStackElement('srbURIReplacement', obj)
            self.stack.append(stackObj)
            done = 1
    ...
    def endElement(self, name):
        ...
        if name == 'XSDataDouble':
            ...
            if len(self.stack) >= 2:
                self.stack[-2].obj.setSrbURIPattern(self.stack[-1].obj)
                self.stack.pop()
                done = 1
        elif name == 'srbURIReplacement':
            if len(self.stack) >= 2:
                self.stack[-2].obj.setSrbURIReplacement(self.stack[-1].obj)
                self.stack.pop()
                done = 1
        elif name == 'xmlIngestFileName':
            if len(self.stack) >= 2:
...
def parseLiteral(inFileName):
    ...
    # Enable Python to collect the space used by the DOM.
    doc = None
    sys.stdout.write('from XSDataPluginExecSRBRegisterv10 import *\n\n')
    sys.stdout.write('rootObj = XSDataDouble(\n')
    rootObj.exportLiteral(sys.stdout, 0, name_="XSDataDouble")
    sys.stdout.write(')\n')
    return rootObj

class XSDataPluginExecSRBRegisterv10:
    pass
