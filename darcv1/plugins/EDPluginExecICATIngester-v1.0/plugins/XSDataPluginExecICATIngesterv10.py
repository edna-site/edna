...
#
# Generated Tue May 12 13:52:32 2009 by EDGenerateDS.py.
#
...
class XSDataInputPluginExecICATIngester(XSDataInput):
    subclass = None
    def __init__(self, command=None, xmlIngestFileName=None, configuration=None):
    ...
    def exportChildren(self, outfile, level, name_='XSDataInputPluginExecICATIngester'):
        for command_ in self.getCommand():
            command_.export(outfile, level, name_='command')
    ...
    def exportLiteralChildren(self, outfile, level, name_):
        ...
        for command in self.command:
            ...
            command.exportLiteral(outfile, level, name_='command')
            showIndent(outfile, level)
            outfile.write('),\n')
        level -= 1
        showIndent(outfile, level)
...
class SaxXSDataDoubleHandler(handler.ContentHandler):
    ...
    def startElement(self, name, attrs):
        ...
        if name == 'XSDataDouble':
            ...
            obj = XSDataString.factory()
            stackObj = SaxStackElement('command', obj)
            self.stack.append(stackObj)
            done = 1
    ...
    def endElement(self, name):
        ...
        if name == 'XSDataDouble':
            ...
            if len(self.stack) >= 2:
                self.stack[-2].obj.addCommand(self.stack[-1].obj)
                self.stack.pop()
                done = 1
        elif name == 'xmlIngestFileName':
            if len(self.stack) >= 2:
...
def parseLiteral(inFileName):
    ...
    # Enable Python to collect the space used by the DOM.
    doc = None
    sys.stdout.write('from XSDataPluginExecICATIngesterv10 import *\n\n')
    sys.stdout.write('rootObj = XSDataDouble(\n')
    rootObj.exportLiteral(sys.stdout, 0, name_="XSDataDouble")
    sys.stdout.write(')\n')
    return rootObj

class XSDataPluginExecICATIngesterv10:
    pass
