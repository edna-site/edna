#
#    Project: The EDNA Full Field XAS Project
...
#	full_path="$(cd "${0%/*}" 2>/dev/null; echo "$PWD"/"${0##*/}")"
	...
	export EDNA_HOME=`dirname "$full_path" | sed 's/\/sandbox\/.*\/datamodel$//'`
	echo "Guessing EDNA_HOME= "$EDNA_HOME
fi

xsDataBaseName=XSDataID11v1_0
xsdHomeDir=${EDNA_HOME}/sandbox/id11/EDPluginControlId11-v1.0/datamodel
pyHomeDir=${EDNA_HOME}//sandbox/id11/EDPluginControlId11-v1.0/plugins
