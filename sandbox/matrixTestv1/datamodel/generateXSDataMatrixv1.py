...
pyHomeDir = os.path.join(os.path.dirname(xsdHomeDir), "src")
includeXSDFilePath = os.path.join(EDNA_HOME, "kernel", "datamodel")
jar = os.path.join(includeXSDFilePath, "EDGenerateDS.jar")
os.system("java -jar %s -includepaths %s -sourceDir %s -sourceFile %s -targetdir %s" % (jar, includeXSDFilePath, xsdHomeDir, xsDataName, pyHomeDir))
