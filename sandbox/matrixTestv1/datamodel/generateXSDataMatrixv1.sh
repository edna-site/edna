...
#	full_path="$(cd "${0%/*}" 2>/dev/null; echo "$PWD"/"${0##*/}")"
	...
	export EDNA_HOME=`dirname "$full_path" | sed 's/\/matrixTestv1\/datamodel$//'`
	echo "Guessing EDNA_HOME= "$EDNA_HOME
fi

xsDataBaseName=XSDataMatrixv1
xsdHomeDir=${EDNA_HOME}/matrixTestv1/datamodel
pyHomeDir=${EDNA_HOME}/matrixTestv1/src
