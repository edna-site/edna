...
#	full_path="$(cd "${0%/*}" 2>/dev/null; echo "$PWD"/"${0##*/}")"
	...
	export EDNA_HOME=`dirname "$full_path" | sed 's/\/bug717\/.*\/datamodel$//'`
	echo "Guessing EDNA_HOME= "$EDNA_HOME
fi

xsDataBaseName=XSDataFind
xsdHomeDir=${EDNA_HOME}/sandbox/bugs/bug717/datamodel
pyHomeDir=${EDNA_HOME}/sandbox/bugs/bug717/src
