...
#	full_path="$(cd "${0%/*}" 2>/dev/null; echo "$PWD"/"${0##*/}")"
	...
	export EDNA_HOME=`dirname "$full_path" | sed 's/\/bioSaxsv1\/datamodel$//'`
	echo "Guessing EDNA_HOME= "$EDNA_HOME
fi

xsDataBaseName=XSDataBioSaxsv1_0
xsDataBaseDir=EDPluginBioSaxs-v1.0
xsdHomeDir=${EDNA_HOME}/bioSaxsv1/datamodel
pyHomeDir=${EDNA_HOME}/bioSaxsv1/src
