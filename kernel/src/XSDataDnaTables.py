...
#
# Generated Tue Apr 21 12:15:29 2009 by EDGenerateDS.py.
#
...
class table:
    subclass = None
    def __init__(self, image=-1, name='', list=None):
        self.image = image
        self.name = name
        if list is None:
            self.list = []
    ...
    def exportAttributes(self, outfile, level, name_='table'):
        if self.getImage() is not None:
            outfile.write(' image="%s"' % (self.getImage(), ))
    ...
    def buildAttributes(self, attrs):
        if attrs.get('image'):
            try:
                self.image = int(attrs.get('image').value)
            except ValueError:
                raise ValueError('Bad integer attribute (image)')
        if attrs.get('name'):
